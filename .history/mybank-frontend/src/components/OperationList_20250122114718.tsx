...
import { getOperations, deleteOperation } from "./api"; // Assurez-vous d'importer les bonnes fonctions API
...
const OperationList: React.FC = () => {
  ...
  const handleDelete = async (id: number) => {
    await deleteOperation(id); // Supprimer l'opération via l'API
    setOperations(operations.filter((op: any) => op.id !== id)); // Mettre à jour la liste localement
  };
